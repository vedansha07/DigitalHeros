  ...
  return (
    <div>
      <div className="mb-8 pb-8 border-b border-cream-border">
        <p className="text-2xs font-black uppercase tracking-[0.3em] text-ink-faint mb-2">New Account</p>
        <h1 className="text-3xl font-black text-ink tracking-tight">Create account.</h1>
        <p className="text-sm text-ink-muted mt-1.5">Join Digital Heros and start making an impact.</p>
      </div>
      ...
      <form onSubmit={handleSignup} className="space-y-4">
        ...
        <div>
          <label htmlFor="full-name" className="block text-xs font-black uppercase tracking-[0.2em] text-ink-muted mb-2">Full Name</label>
          <input id="full-name" type="text" required value={fullName} onChange={e => setFullName(e.target.value)}
            className={inputCls} placeholder="John Smith" autoComplete="name" />
        </div>
        ...
        <div>
          <label htmlFor="password" className="block text-xs font-black uppercase tracking-[0.2em] text-ink-muted mb-2">Password</label>
        ...
        <div>
          ...
          <div className="relative">
            <input id="confirm-password" type={showPw ? "text" : "password"} required value={confirmPassword}
              ...
              className={`${inputCls} pr-12 ${confirmPassword.length > 0 && !pwMatch ? "border-coral focus:border-coral focus:ring-coral" : pwMatch ? "border-lime focus:border-lime focus:ring-lime" : ""}`}
        ...
        <button type="submit" disabled={loading}
          ...
          {loading ? <><Loader2 size={14} className="animate-spin" /> Creating...</> : <>Create Account <ArrowUpRight size={14} /></>}
        </button>
      </form>
